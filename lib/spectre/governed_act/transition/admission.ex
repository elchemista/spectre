defmodule Spectre.GovernedAct.Transition.Admission do
  ...
  Candidate reconstruction and `authorize_candidate/5` are also used when an
  ...
  alias Spectre.GovernedAct.Transition.Admission.Presentation, as: PresentationReplay
  ...
  def apply(
    ...
    with {:ok, decision} <- Record.decode(Spectre.Decision, data),
      ...
      case Map.fetch(projection.admissions, candidate_key) do
        :error ->
          {:ok,
           %{
             projection
             | decisions: Map.put(projection.decisions, identity, decision),
               admissions:
                 Map.put(projection.admissions, candidate_key, %{
        ...
        {:ok, %{decision_ref: existing_ref}} ->
          case Map.fetch(projection.decisions, existing_ref) do
            {:ok, %Decision{candidate_digest: ^candidate_digest}} ->
            ...
            :error ->
              {:error, {:candidate_identity_decision_not_found, candidate_key, existing_ref}}
          end
  ...
  def apply(
    ...
    with {:ok, act} <- Record.decode(Spectre.Act, data),
         ...
         :ok <- one_act_per_decision(projection, decision),
         :ok <- match_act_to_decision(projection, act, decision),
         {:ok, candidate} <- Binding.candidate(act),
         :ok <- validate_candidate_at_act(projection, candidate, act, decision) do
      admission = Map.fetch!(projection.admissions, decision.candidate_identity_key)
      ...
      {:ok,
       %{
         ...
         | acts: Map.put(projection.acts, identity, act),
           admissions:
             Map.put(projection.admissions, decision.candidate_identity_key, %{
               admission
               | act_ref: identity
             })
       }}
  ...
  defp validate_decision_context(projection, decision) do
    with {:ok, context} <- decision_submission_context(decision),
      ...
      profile = State.host_profile(projection)
      surface = State.surface(projection)
  ...
  defp match_act_to_decision(projection, act, decision) do
    ...
    admission = Map.get(projection.admissions, act.candidate_identity_key)
  ...
  defp one_act_per_decision(projection, decision) do
    case Map.fetch(projection.admissions, decision.candidate_identity_key) do
  ...
  defp validate_candidate_at_act(projection, candidate, act, decision) do
    ...
    with {:ok, mandate} <- Index.fetch_mandate(projection, act.mandate_ref),
         ...
         {:ok, presentation_basis_refs} <-
           PresentationReplay.validate(projection, candidate, act, available_evidence),
         :ok <-
  ...
  defp validate_surface_binding(%State{} = projection, candidate, act) do
  ...
  defp validate_candidate_recognition(projection, candidate, decision, mandate, evidence) do
