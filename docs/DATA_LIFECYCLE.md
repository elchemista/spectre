...
| Installed package Instance data | exact Instance Ref plus installation identity | Stack installable | yes when both package-data erasure callbacks are implemented |
...
Instance Ref. Package-data callbacks receive that Ref and immutable installation
identity, not a raw Subject value. Definition artifacts are shared and therefore
stay outside subject erasure.
...
journal, receipt-payload, installed package-data, and checkpoint callbacks are
present. `Spectre.Doctor` exposes the core adapter posture as read-only
operational checks.
