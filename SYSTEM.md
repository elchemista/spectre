...
Spectre `0.3.4` is an OTP-native kernel for building conversational and
...
## Core 0.3.4 and satellite compatibility
...
assumed compatible with core `0.3.4` until their manifests and adapter suites
...
packages at the `0.3.4` core and treat a compile as proof of compatibility.
...
repository under the same constraint. New satellites targeting core `0.3.4`
must instead declare that requirement in their Installable manifest and test
against the local `0.3.4` checkout plus the published package independently.
